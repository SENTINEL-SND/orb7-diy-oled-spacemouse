...
private:
    ...
    // FIXED Problem 2: Parameter size corrected to exactly match the 12-byte payload array
    bool jiggleValues(uint8_t val[12], bool lastBit);

    SpaceMouseHIDStates nextState;
#if (NUMKEYS > 0)
    // FIXED: Removed obsolete bitNumber mapping array declaration to optimize static SRAM [7]
