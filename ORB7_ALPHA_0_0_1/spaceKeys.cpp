...
// Mudança para const uint8_t para conservar memória RAM dinâmica
...
// Function to read and store the digital states for each of the keys using 8-bit types [6]
...
// Evaluate and debounce keys using an Instant Press and Delayed Release approach [2]
void evalKeys(uint8_t *keyVals, uint8_t *keyOut, uint8_t *keyState) {
  // Static timestamp array utilizing uint16_t to reduce SRAM memory footprint
  ...
  for (uint8_t i = 0; i < NUMKEYS; i++) {
    ...
    if (!keyVals[i]) { // Physical button is PRESSED
      ...
      if (keyState[i] == 1) { 
        // Only register the release once the physical signal is stably high for DEBOUNCE_KEYS_MS
